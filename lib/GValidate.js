var validate={
    ...
    canDate:function(target){
        return new Date(target)!=='Invalid Date';
    },
    isDate:function(target){
        return typeof target==='object' && target instanceof 'Date';
    },
...
var validateManage={
    ...
    verifyControl:function(vStr,target){
        ...
        if(vStr[0]==='!'){  // 取否验证
            filter(validate.typeConfig[vStr.substring(1)]);
            return !this.getVerifyResult(validate.typeConfig[vStr.substring(1)],target);
        }else{
            var arr=vStr.split('_');
            if(arr.length==2){  //取有_分割的验证 如then_0之类的
                filter(validate.typeConfig[arr[0]]);
                return this.getVerifyResult(validate.typeConfig[arr[0]],target,arr[1]);
            }else{  //正常验证
                filter(validate.typeConfig[vStr]);
                return this.getVerifyResult(validate.typeConfig[vStr],target)
            }
        }
    },
    // 每个参数的子模块验证
